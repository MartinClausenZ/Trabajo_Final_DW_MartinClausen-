...
## Sprint 7 — Dinámica de Estrella de Mar

### 1. Comenzar a hacer
- Verificar el orden de los middlewares en las rutas — express-validator debe ir DESPUÉS de multer para que req.file esté disponible en las validaciones custom de imagen.
- Testear las validaciones back-end con Postman enviando datos inválidos antes de conectar las vistas.
- Revisar que el frontend JS y el backend validan las mismas reglas — sino se generan inconsistencias.

### 2. Hacer más
- Usar el patrón IIFE para las validaciones front-end, encapsulando estado y evitando contaminación del scope global.
- Mostrar errores cerca del campo correspondiente en las vistas — mejora mucho la UX respecto de un bloque de errores genérico.
- Agregar feedback visual inmediato (bordes verdes/rojos) en cada campo mientras el usuario escribe.
...
### 4. Hacer menos
- Dejar la validación front-end para el final — conviene implementarla en paralelo con la back-end para detectar diferencias de reglas a tiempo.
- Acumular muchos cambios sin verificar que los errores se muestran correctamente en las vistas.

### 5. Dejar de hacer
- Confiar en la validación HTML5 nativa (required, pattern) — siempre agregar novalidate y manejar todo con JavaScript custom.
- Asumir que express-validator funciona sin verificar el req.file — las validaciones custom de imagen necesitan que multer ya haya procesado el archivo.